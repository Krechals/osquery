  ...
  for (fs::directory_iterator entry(
    ...
    if (is_dir) {
      ...
      std::string canonical_path =
          fs::canonical(entry_path, canonical_ec).string();
      if (!canonical_ec) {
        dfsTraverseDirectories(
            canonical_path, visited_dirs, results, depth + 1);
      }
    }
  }
}
...
static void genGlobs(std::string path,
  ...
  if (should_expand) {
    ...
    for (const auto& base_dir : initial_results) {
      ...
      boost::system::error_code canonical_ec, ignored_ec;
      std::string canonical_dir =
          fs::canonical(base_dir, canonical_ec).string();
      if (!canonical_ec && fs::is_directory(canonical_dir, ignored_ec)) {
        dfsTraverseDirectories(canonical_dir, dir_inos, results, 0);
      }
    }
  } else {
